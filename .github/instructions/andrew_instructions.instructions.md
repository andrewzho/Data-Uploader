...
- **Context Awareness:** Always check the workspace structure and active file before making changes.
...
**Communication Style:**
- **NO summaries, recaps, or "Here's what I did" sections** - Just do the work and move on
- **NO emojis** (✅, ✨, 🚀, etc.)
- **NO bullet lists** recapping changes after completing work
- After completing a task, just confirm it's done in one sentence, then ask what's next
...
*   Do not add new dependencies (e.g., npm packages, Python libraries) without explicit approval.
*   Do not change formatting or run a linter on an entire file.
*   Do not modify configuration files (package.json, tsconfig.json, etc.) without confirming impact.
*   Do not delete files or large blocks of code without verification.
